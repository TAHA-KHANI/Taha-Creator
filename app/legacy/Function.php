...
function random2($string,$length){
//create a or array string called chars
        $chars =$string;
         $str = "";
        //is the variable which is equal to the length of the string called chars
...
function getrss($url){
$rss = simplexml_load_string(tc_http($url), "SimpleXMLElement", LIBXML_NONET);
...
function unsetall($array, $key){
    unset($array[$key]);


    foreach($array as $k=>$v){
        if($v == $key){
            preg_match("/into(.*)/", $k, $find);
            unset($array[$find[1]]);
        }
    }
    ...
    while(true){
        $end = 1;
        foreach($array as $k=>$v){
            if(empty($array[$v]) && preg_match("/into(.*)/", $k, $find)){
                unset($array[$k]);
                unset($array[$v]);
                unset($array[$find[1]]);
                $end = 0;
