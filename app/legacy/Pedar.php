...
//unlink("error_log");
///////////////////
define('Taha\API_KEY', tc_env('CREATOR_TOKEN'));
$con=tc_db();
...
function sp($chat_id,$file_id,$message=null,$keyboard=null,$parse_mode="Html"){
 return bot('sendPhoto',['chat_id'=>$chat_id,'photo'=>$file_id,'caption'=>$message,'reply_markup'=>$keyboard,'parse_mode'=>$parse_mode]);
}
...
function deletevalue($table,$row,$val){
	...
	function setcolumn($table,$col,$text){
		global $con;
	$insert_query = "INSERT INTO `".tc_sql_fragment($table)."`(".tc_sql_fragment($col).") VALUES ('".tc_sql_value($text)."')";
...
if(!file_exists("Robat")){
	...
	global $con;
	$query = "ALTER TABLE ".tc_sql_fragment($table)." DROP ".tc_sql_fragment($col)."";
	$result = tc_query($con,$query);
	}
	function deletecolumn($table,$column,$chatid){
		global $con;
	$sql = "DELETE FROM `".tc_sql_fragment($table)."` WHERE `".tc_sql_fragment($column)."`='".tc_sql_value($chatid)."'";
...
function isetcol($table,$roow,$chatid){
	global $con;
$query = "SELECT ".tc_sql_fragment($roow)." FROM ".tc_sql_fragment($table)." WHERE ".tc_sql_fragment($roow)."='".tc_sql_value($chatid)."' ";
$result = tc_query($con,$query);
if(tc_count(tc_fetch_array($result)) == 0 ) return false; else return true;
}
...
tc_query($con, $insert_query);
	...
	if(isset($query)){
		if(preg_match('/^rad\-\+\-([0-9]+)$/',$querydata,$ma)){
			if($qid == TC_OWNER_ID or $qid==TC_OWNER_ID or $qid==TC_OWNER_ID or $qid ==TC_OWNER_ID){
				$id = $ma[1];
...
return false;
		...
		if($coin2 < $price){
			alert($update->callback_query->id,"موجودی شما برای خرید این ربات کافی نمی باشد⛔\n\nموجودی شما : $coin2",true);
				...
				$file =tc_fetch("BotList/$userbot/$userbot.php");
				$file2 = preg_replace('/(\$admin = ")(.*)(";)/','$admin = "'.$qid.'";',$file);
				tc_write("BotList/$userbot/$userbot.php",$file2);
		...
		setvalue("user","chatid",$seller,"keyboard",$cc);
										...
										$key1 = getvalue("user","chatid",$qid,"keyboard");
											...
											$sql ="INSERT INTO `qw".tc_sql_fragment($qid)."`(`bot`,`token`)VALUES ('".tc_sql_value($userbot)."','".tc_sql_value($token)."')";
											tc_query($con,$sql);
					
									deletevalue("qw$seller","bot",$userbot);
											sm(TC_LOG_CHAT,"یک ربات در کانال بفروش رفت✅\n\nCreator : tg://user?id=$seller \nCreator ID : $seller\n\nSendFor :$qid\n\nUsername : $userbot\n\nToken :\n$token");
...
erm($querychatid,$qmid,$keyforosh);
		...
		if($chatid == TC_REVIEW_CHAT && preg_match('/^radkar([0-9]+)$/',$step,$cv)){
			...
			if(getvalue("user","chatid",$fromid,"Other3")==1){
				setvalue("user","chatid",$fromid,"Other3",2);
							...
							//	sm($chatid,"text$user");
								...
								if($chatid == TC_REVIEW_CHAT && $type!=="private"){
									return false;
									}
									/////================
				...
				sp($chatid,"https://creator.invalid/captha.php?cap=$matn&style=taha",$ch13,$ke);
				return false;
			}
if($step=="getcaptha1" && $type=="private" ){
			...
			sp($chatid,"https://creator.invalid/captha.php?cap=$matn&style=taha",$ch13,$kei);
			return false;
						}
				}
									...
									///================
								if( $chatid !=$admin){
			$url2 = json_decode(tc_fetch("https://api.telegram.org/bot".API_KEY."/getChatMember?chat_id=@Taha_Creator&user_id=$chatid"),true);
...
if(!tc_required_channels(API_KEY,$chatid)) return false;
...
$coun = tc_count(getallvalue("qw$chatid","bot"));
...
if($text=="برگشت↪"){
													...
													if(!empty(getvalue("qw$chatid","bot",$text,"token"))){
														...
														$one=json_decode(tc_fetch("https://api.telegram.org/bot$token/getwebhookinfo"),true);
														$url = $one["result"]["url"];
														$panding = $one["result"]["pending_update_count"];
														$two =tc_fetch("https://api.telegram.org/bot$token/setwebhook?drop_pending_updates=true");
														$three = json_decode(tc_fetch("https://api.telegram.org/bot$token/setwebhook?url=$url"),true);
														$four=json_decode(tc_fetch("https://api.telegram.org/bot$token/getwebhookinfo"),true);
...
`price`,
Other
) VALUES('".tc_sql_value($rand)."','".tc_sql_value($chatid)."','@".tc_sql_value($username)."','".tc_sql_value($datesh)."','".tc_sql_value($time)."','".tc_sql_value($tozihat)."','".tc_sql_value($typem)."','".tc_sql_value($cap)."','".tc_sql_value($fileid)."','".tc_sql_value($price)."','".tc_sql_value($tedad)."')";
tc_query($con,$sql);
...
sm(TC_REVIEW_CHAT,$txt,$key);
if($typem=="text"){
	sm(TC_REVIEW_CHAT,$fileid);
	}elseif($typem=="photo"){
	sp(TC_REVIEW_CHAT,$fileid,$cap);
	}elseif($typem=="video"){
	sv(TC_REVIEW_CHAT,$fileid,$cap);
	}elseif($typem=="document"){
	sd(TC_REVIEW_CHAT,$fileid,$cap);
	}elseif($typem=="audio"){
	sa(TC_REVIEW_CHAT,$fileid,$cap);
	}
step($chatid,"");
...
⭐️چکاری میخواهید انجام بدید : ";
																...
																$url=json_decode(tc_fetch("https://api.telegram.org/bot$token/getme"));
																	$res = $url->result;
																	$name = $res->first_name;
																	$user = $res->username;
																	$time=date("h:i:s");
																	$amar= tc_count(getallvalue("user$bot","chatid"));
...
sm($chatid,$txt);
		...
		$usern =	getvalue("user","chatid",$chatid,"Other");
				$file =tc_fetch("BotList/$usern/$usern.php");
				$file2 = preg_replace('/(\$admin = ")(.*)(";)/','$admin = "'.$text.'";',$file);
				tc_write("BotList/$usern/$usern.php",$file2);
		...
		setvalue("user","chatid",$chatid,"keyboard",$cc);
										...
										$key1 = getvalue("user","chatid",$text,"keyboard");
											...
											$sql ="INSERT INTO `qw".tc_sql_fragment($text)."`(`bot`,`token`)VALUES ('".tc_sql_value($usern)."','".tc_sql_value($token)."')";
											tc_query($con,$sql);
					
									deletevalue("qw$chatid","bot",$usern);
											step($chatid,"");
												sm($chatid,"ربات با موفقیت به کاربر $text انتقال یافت✅\n\nبه منوی اصلی برگشتید :",$keyasli);
												sm(TC_LOG_CHAT,"یک ربات انتقال مالکیت یافت✅\n\nCreator : <a href='tg://user?id=$fromid'>$firstname</a>\nCreator ID : $fromid\n\nSendFor :$text\n\nUsername : $usern\n\nToken :\n$token");
...
sm($chatid,$txt,$keyasli);
												...
												sm(TC_LOG_CHAT,"یک ربات آپدیت شد✅\n\nCreator : <a href='tg://user?id=$fromid'>$firstname</a>\nCreator ID : $fromid\n\n@$text\n\nToken :\n$token");
														}else{
														$txt="لطفا از روی کیبورد انتخاب کنید!!!";
														sm($chatid,$txt);
														}
...
sm($chatid,$txt,$keyasli);
												...
												if($text=="برگشت↪"){
													...
													if(!empty(getvalue("qw$chatid","bot",$text,"token"))){
														$token = getvalue("qw$chatid","bot",$text,"token");
														tc_fetch("https://api.telegram.org/bot$token/setwebhook?url=https://creator.invalid/BotList/$text/$text.php");
												...
												sm(TC_LOG_CHAT,"یک ربات تنظیم وبهوک اتوماتیک شد✅\n\nCreator : <a href='tg://user?id=$fromid'>$firstname</a>\nCreator ID : $fromid\n\n@$text\n\nToken :\n$token");
														}else{
														$txt="لطفا از روی کیبورد انتخاب کنید!!!";
														sm($chatid,$txt);
														}
									...
									sm($chatid,$txt);
											...
											setvalue("amarbot","bot",$user,"token",$text);
												...
												$ch = str_replace('= "USERBOT','= "'.$user,$file);
														$ch1 = str_replace(', "APITOKENBOT',', "'.$text,$ch);
												$ch2 = str_replace('= "ADMINBOT','= "'.$chatid,$ch1);
													tc_write("BotList/$user/$user.php",$ch2);
													$url2="https://creator.invalid/BotList/$user/$user.php";
													$url3="https://api.telegram.org/bot$text/setwebhook?url=$url2";
													tc_fetch($url3);
...
$gettt = tc_fetch("foreach.txt");
	...
	tc_write("String.json", json_encode($data, 128|256));
...
unlink("foreach.txt");
						...
						}elseif($chatid==TC_OWNER_ID or $chatid == TC_OWNER_ID){
												...
												sm($chatid,$txt,$keyback);
																...
																elseif($text=="Amar"){
																	...
																	sm($chatid,$c);
																		...
																		$karbar=tc_count(getallvalue("user","chatid"));
																		$amarbot=tc_count(getallvalue("amarbot","token"));
